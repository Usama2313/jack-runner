...
export const Player = ({ playerZRef }) => {
  ...
  useFrame((state, delta) => {
    ...
    const { lane, isJumping, isRolling, isDead, activePowerups, setJumping, setRolling } = store;
    ...
    const targetBank = isDead ? 0 : -lateralVelocity * 0.035;
    currentBankZRef.current = THREE.MathUtils.lerp(currentBankZRef.current, targetBank, Math.min(1, delta * 12));
    ...
    // Death animation
    const targetRotZ = isDead ? 1.5 : 0;
    const targetRotX = isDead ? -1.3 : 0;
    const rotSpeed = isDead ? 4 : 10;
    ...
    if (!isDead && !isJetpackActive) {
      ...
      // Wind scarf physical wave motion
    ...
    if (chestCoreRef.current) {
      const pulse = 1.2 + Math.sin(state.clock.elapsedTime * 8) * 0.5;
    ...
    <group ref={groupRef}>
      ...
      <group ref={innerModelRef}>
        ...
        <group ref={headRef} position={[0, 1.44, 0]}>
          ...
          {/* Dynamic Cyber Headset Ears */}
          <mesh position={[-0.27, 0.02, 0]}>
            <cylinderGeometry args={[0.07, 0.07, 0.08, 12]} />
            <meshStandardMaterial color="#0284c7" emissive="#0284c7" emissiveIntensity={1.0} metalness={0.8} />
          </mesh>
          <mesh position={[0.27, 0.02, 0]}>
            <cylinderGeometry args={[0.07, 0.07, 0.08, 12]} />
            <meshStandardMaterial color="#0284c7" emissive="#0284c7" emissiveIntensity={1.0} metalness={0.8} />
          ...
          {/* Glowing Cyber Eyes HUD Scanline inside Visor */}
          ...
          {/* Futuristic Collar / Bandana */}
          <mesh position={[0, -0.16, 0]}>
            <cylinderGeometry args={[0.18, 0.22, 0.12, 16]} />
            <meshStandardMaterial color={character.capColor || '#dc2626'} roughness={0.3} metalness={0.2} />
          </mesh>
        </group>

        {/* ─── DYNAMIC FLOWING WIND SCARF ──────────────────────────────────── */}
        <group position={[0, 1.26, -0.18]}>
          <group ref={scarf1Ref}>
            <mesh position={[0, -0.18, -0.15]}>
              <boxGeometry args={[0.24, 0.35, 0.03]} />
              <meshStandardMaterial color={character.capColor || '#ec4899'} emissive={character.capColor || '#ec4899'} emissiveIntensity={0.6} />
            </mesh>
            <group ref={scarf2Ref} position={[0, -0.32, -0.15]}>
              <mesh position={[0, -0.18, -0.12]}>
                <boxGeometry args={[0.2, 0.38, 0.02]} />
                <meshStandardMaterial color={character.capColor || '#ec4899'} emissive={character.capColor || '#ec4899'} emissiveIntensity={0.8} />
              </mesh>
            </group>
          </group>
        </group>
        ...
        <group position={[0, 0.92, 0]}>
          {/* Main Contoured Chest Armor */}
          <mesh castShadow>
            <boxGeometry args={[0.54, 0.62, 0.34]} />
            <meshStandardMaterial color={character.shirtColor || '#2563eb'} roughness={0.3} metalness={0.6} />
          </mesh>

          {/* Shoulder Armor Pauldrons */}
          <mesh position={[-0.32, 0.24, 0]} castShadow>
          ...
          {/* Central Chest Arc Reactor Core */}
          <mesh position={[0, 0.08, 0.17]}>
            <cylinderGeometry args={[0.11, 0.11, 0.04, 16]} rotation={[Math.PI / 2, 0, 0]} />
            <meshStandardMaterial color="#0f172a" metalness={0.9} />
          </mesh>
          <mesh ref={chestCoreRef} position={[0, 0.08, 0.19]}>
          ...
          <mesh position={[0, -0.28, 0]}>
            <boxGeometry args={[0.56, 0.09, 0.36]} />
        ...
        {/* Left Arm */}
        <group ref={leftArmRef} position={[-0.34, 1.12, 0]}>
          <mesh position={[0, -0.24, 0]} castShadow>
            <cylinderGeometry args={[0.085, 0.075, 0.48, 10]} />
            <meshStandardMaterial color={character.shirtColor || '#2563eb'} roughness={0.3} metalness={0.5} />
          </mesh>
          {/* Cyber Gauntlet */}
        ...
        {/* Right Arm */}
        <group ref={rightArmRef} position={[0.34, 1.12, 0]}>
          <mesh position={[0, -0.24, 0]} castShadow>
            <cylinderGeometry args={[0.085, 0.075, 0.48, 10]} />
            <meshStandardMaterial color={character.shirtColor || '#2563eb'} roughness={0.3} metalness={0.5} />
          </mesh>
        ...
        {/* ─── ARTICULATED LEGS & KINETIC THRUSTER SNEAKERS ────────────────── */}
        {/* Left Leg */}
        <group ref={leftLegRef} position={[-0.16, 0.58, 0]}>
          <mesh position={[0, -0.26, 0]} castShadow>
            <cylinderGeometry args={[0.095, 0.085, 0.54, 10]} />
            <meshStandardMaterial color={character.pantsColor || '#0f172a'} roughness={0.4} metalness={0.4} />
          </mesh>
          {/* Kinetic Cyber Sneaker */}
        ...
        {/* ─── FUTURISTIC AERODYNAMIC HOVERBOARD ───────────────────────────── */}
        <group ref={hoverboardMeshRef} visible={false} position={[0, -0.16, 0]}>
          ...
          {/* Underglow Anti-Gravity Plasma Ring */}
          ...
          {/* Dual Rear Turbine Thrusters */}
          <mesh position={[-0.22, 0, 0.72]}>
            <cylinderGeometry args={[0.07, 0.09, 0.2, 12]} rotation={[Math.PI / 2, 0, 0]} />
            <meshStandardMaterial color="#0f172a" metalness={0.9} />
          </mesh>
          <mesh position={[-0.22, 0, 0.83]}>
        ...
        {/* ─── POWERUP TRANSFORMATION VISUALS ──────────────────────────────── */}
        {/* Jetpack with Animated Thruster Flames */}
        ...
        {/* Magnet Holographic Plasma Aura */}
        ...
        {/* Super Sneakers Glowing Boot Wings */}
        <group ref={sneakersMeshRef} visible={false} position={[0, 0.1, 0]}>
          <mesh position={[-0.28, 0, 0]} rotation={[0, 0, 0.3]}>
            <planeGeometry args={[0.3, 0.4]} />
            <meshStandardMaterial color="#10b981" emissive="#10b981" emissiveIntensity={1.6} side={THREE.DoubleSide} />
          </mesh>
          <mesh position={[0.28, 0, 0]} rotation={[0, 0, -0.3]}>
            <planeGeometry args={[0.3, 0.4]} />
            <meshStandardMaterial color="#10b981" emissive="#10b981" emissiveIntensity={1.6} side={THREE.DoubleSide} />
        ...
        {/* 2X Multiplier Lightning Shield */}
